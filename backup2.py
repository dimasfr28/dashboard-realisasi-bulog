...
def create_summary_table(df_filtered, df_target):
    ...
    def get_kanwil_data(kanwil):
        ...
        gkg = df_kanwil[
            ...
            (df_kanwil['spesifikasi'].astype(str).str.contains('GKG 14%', case=False, na=False))
        ]['In / Out'].sum() / 1000
...
#     """Create line chart with 3 lines (BERAS, GKP, GKG) based on In/Out and Tanggal Penerimaan"""
#     # Filter data untuk BERAS (BERAS MEDIUM + BERAS PREMIUM)
...
#     # Filter data untuk GKP (Gabah dengan spesifikasi GKP)
#     df_gkp = df_filtered[
...
#     # Filter data untuk GKG (Gabah dengan spesifikasi GKG - bisa GKG 14%, GKG 16%, dll)
#     df_gkg = df_filtered[
#         (df_filtered['Komoditi'] == 'GABAH') &
...
#             size=10,
#             color='#4bacc6',
#             line=dict(color="#759ac9", width=2)
#         ),
#         text=[f'{val:,.1f}' for val in daily_beras['In Out (Ton)']],
...
#         ),
#         text=[f'{val:,.1f}' for val in daily_gkp['In Out (Ton)']],
#         textposition='top center',
#         textfont=dict(size=10, color='#000000', family='Arial Black'),
...
#             size=10,
#             color='#5cb85c',
#             line=dict(color="#4cae4c", width=2)
#         ),
#         text=[f'{val:,.1f}' for val in daily_gkg['In Out (Ton)']],
#         textposition='top center',
#         textfont=dict(size=10, color='#000000', family='Arial Black'),
...
#     fig.update_layout(
#         title=None,
#         xaxis_title='Tanggal Penerimaan',
#         yaxis_title='In / Out (Ton)',
#         height=450,
#         hovermode='x unified',
...
def main():
    ...
    with colA:
        ...
        all_akun_analitik = sorted([a for a in df['Akun Analitik'].unique() if pd.notna(a)])
    ...
    with colB:
        st.markdown("#### Kanwil")
        all_kanwil = sorted([k for k in df['kanwil'].unique() if pd.notna(k)])
    ...
    if not df_kancab.empty:
        ...
        if not kancab_df.empty:
            ...
            st.markdown(html_kancab, unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    # Footer
    st.markdown("---")
    st.markdown(f"<div style='text-align: center; color: #1f497d;'><i>📅 Data terakhir diupdate: {datetime.now().strftime('%d %B %Y, %H:%M:%S WIB')}</i></div>", unsafe_allow_html=True)
